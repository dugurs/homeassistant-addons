...
### 수정 (Fix) — beta.119의 "중지"가 실제 채팅에서는 전혀 인식되지 않던 문제
- **재현**: beta.119를 mock 테스트로 검증할 때는 `execute_device_control_intent()`를 직접 호출해서 통과했는데, 실제 `/api/chat`으로 "안방 스피커 중지해줘"를 보내면 "무슨 말씀인지 파악하지 못했습니다"로 나옴
- **원인**: `core/ha_engine.py`의 `handle_agent_chat()`이 기기 제어 함수를 호출하기 *전에* "제어 동사가 하나라도 있는지" 먼저 걸러내는 관문이 있는데, 그 목록에 "정지"는 있어도 "중지"는 없었음(정지=jeong-ji vs 중지=jung-ji, 철자가 다른 별개 단어) -- 그래서 `execute_device_control_intent()`까지 아예 도달하지 못하고 관문에서 걸러짐. 직접 호출하는 mock 테스트는 이 관문을 거치지 않아 문제를 놓쳤음
- **수정**: 그 목록에 "중지"/"멈춰"/"일시정지" 추가
- 실제 `/api/chat`으로 "안방 스피커 중지해줘"/후속 "중지해줘"/"재생해줘" 전부 정상 동작(일시정지 → 정확한 위치에서 재개) 확인, "안방 등 정지해줘"(조명, media_player 아님)는 기존처럼 전원 끄기로 동작해 회귀 없음 확인

## 1.1.0-beta.119

### 수정 (Fix) — 타임바 미표시 + "중지"/"재생"으로 일시정지·재개 지원
- **재현 1 (타임바)**: "안방에 음악 틀어줘" 후 카드에 제목/아티스트는 나오는데 진행 시간바는 안 보임. 원인은 명령이 원래 찾은 "안방 스피커"(`media_player.bed_speaker`, 캐스트 통합 엔티티)가 재생 중인 곡의 `media_title`/`media_artist`/`media_position`은 그대로 반영하면서도 `media_duration`만 아예 속성에 없다는 것(실기기 확인) -- Music Assistant 쪽 엔티티(`media_player.ma_bed_speaker`)에만 `media_duration`이 있음
- **수정 1**: `build_device_cards()`가 media_duration/media_position 중 하나라도 없으면 `find_music_assistant_sibling()`으로 찾은 MA 엔티티에서 부족한 값을 채워 넣음 -- 카드가 어느 엔티티를 기준으로 만들어졌는지는 그대로 두고(토글/볼륨은 계속 원래 스피커 엔티티 기준), 진행바에 필요한 값만 보충
- **재현 2**: "안방에 음악 틀어줘"로 재생 시작 후 "중지"라고 하면 반응이 없거나(트리거 키워드 없음) "안방 스피커 꺼줘"처럼 스피커 전원을 완전히 끄는 `turn_off`가 호출됨 -- 다시 "재생"이라고 해도 그냥 스피커만 켜질 뿐 음악은 이어지지 않음
- **수정 2**: "중지"/"정지"/"멈춰"/"일시정지"는 media_player 대상일 때 `turn_off` 대신 `media_pause`, "재생"은 `turn_on` 대신 `media_play`를 호출하도록 분기 추가 -- 명시적으로 "스피커"를 언급한 경우(`_h_media()`)와 방금 튼 스피커를 그냥 "중지"/"재생"이라고만 부르는 후속 명령(bare follow-up fallback) 둘 다 적용. 실기기 확인 결과 캐스트 엔티티의 `media_pause`/`media_play`가 Music Assistant 큐에도 그대로 반영돼(양쪽 엔티티 상태 동기화 확인) 엔티티를 바꿔 탈 필요 없이 원래 스피커 엔티티에 그대로 호출하면 됨. TV는 기존 IR 스크립트 on/off 경로 그대로 두고 이 오버라이드 대상에서 제외
- 실기기로 재생 중 `media_pause`(일시정지, 재생 위치 보존) → `media_play`(정확히 그 지점부터 재개) 확인, `media_seek`으로 위치 이동도 재확인. "정지"가 조명 등 media_player가 아닌 대상에는 기존처럼 그냥 전원 끄기로 동작하는 것 회귀 없음 확인

## 1.1.0-beta.118

### 추가 (Feature) — 미디어 플레이어 "재생 중" 위젯 + 화면에 보이는 카드만 실시간 갱신
- **"재생 중" 위젯**: `build_device_cards()`가 media_player 카드에 `media_title`이 있을 때만(즉 뭔가 재생 중일 때만) 제목/아티스트/커버/재생시간을 추가로 담도록 확장. `core/ui/scripts.py`에 `nowPlayingHTML()`을 새로 만들어 커버 이미지 + 제목 + 아티스트 + 탐색바(드래그하면 `media_player.media_seek` 호출) + 이전곡/재생·일시정지/다음곡 버튼을 볼륨 슬라이더 위에 렌더링. 커버는 `entity_picture_local`(HA 자체 origin 기준 상대경로 -- 이 채팅 UI는 애드온 자체 origin이라 그대로 쓰면 404)이 아니라 절대 URL인 `entity_picture`를 사용. `ALLOWED_CARD_SERVICES`에 `media_play`/`media_pause`/`media_next_track`/`media_previous_track`/`media_seek` 추가
- **화면에 보이는 컨트롤 카드만 실시간 갱신**: 재생 진행바뿐 아니라 조명/팬/커튼 등 다른 도메인 카드도 다른 경로(물리 스위치 등)로 상태가 바뀌면 화면엔 반영이 안 되던 문제 -- `IntersectionObserver`로 각 `.device-card`가 실제 화면에 보일 때만 3초 간격으로 `refreshDeviceCard()`를 돌리고, 스크롤에서 벗어나면 그 카드의 타이머를 바로 정지. 채팅 기록에 쌓인 모든 카드를 일괄 폴링하는 게 아니라 지금 눈에 보이는 것만 -- 요청대로 범위를 좁힘. `refreshDeviceCard()`가 매번 카드 DOM을 통째로 다시 그리기 때문에(outerHTML 교체), 갱신할 때마다 새로 생긴 노드를 옵저버에 다시 등록하도록 처리 -- 안 하면 첫 갱신 이후로 다시는 추적이 안 됨
- 실기기로 "재생 중" 위젯의 제목/아티스트/커버 노출 확인, 화면에 보이는 카드만 3초마다 갱신되고 스크롤에서 벗어나면 멈추는 것 확인

## 1.1.0-beta.117

### 수정 (Fix) — beta.116의 플레이리스트 컴팩트 목록을 그리드로 되돌림
- 사용자 피드백: 이전 썸네일 그리드(56px 정사각 카드)가 beta.116의 컴팩트 한 줄 목록보다 낫다고 판단 -- `.playlist-card-items`/`.playlist-card-item`/`.playlist-card-thumb`을 원래 그리드 레이아웃으로 복원(스크롤 가능한 최대 높이는 유지해 9개가 와도 카드가 과하게 커지지는 않음). 헤더의 스피커 선택 드롭다운, 9개 표시 등 beta.116의 다른 변경은 그대로 유지

## 1.1.0-beta.116

### 개선 (Improvement) — 플레이리스트 카드 다듬기: 9개까지 표시, 컴팩트 목록, 스피커 선택 추가
- **최근 플레이리스트 개수 6→9개**: `get_recent_playlists()` 기본값 변경. 겸사겸사 [music-assistant.io 공식 문서](https://www.music-assistant.io/)의 Get Library Action 예제를 확인해보니 `get_library` 서비스는 `limit`을 최상위 필드로 바로 받는 게 맞았음(이전엔 `{"pagination": {"limit": N}}`으로 시도해서 400 오류가 났던 것 -- 지금은 클라이언트에서 자른 목록만 쓰고 있었어서 기능상 문제는 없었지만, 서버 쪽에서 애초에 9개만 요청하도록 정리)
- **커버 아트 축소, 그리드 → 컴팩트 목록**: 기존엔 큰 정사각 썸네일(56px) 그리드였는데, 한 줄에 작은 앨범아트(32px)+이름을 나란히 배치하는 스크롤 목록으로 변경 -- 9개가 와도 카드가 과하게 커지지 않고 최대 높이 안에서 스크롤됨
- **스피커 선택 추가**: 공식 문서(Player Providers 페이지)를 확인해 "player provider"의 정확한 의미(기기 종류별로 여러 스피커를 한 번에 관리하는 백엔드 -- Cast/AirPlay/Sonos 등)와 "MA 자체 플레이어 엔티티가 있으면 HA 네이티브 엔티티 대신 그걸 쓰는 게 공식 권장"이라는 점을 재확인함(beta.113의 접근이 문서상으로도 맞는 방향이었음 확인). `core/ha_registry.py`에 `get_all_music_assistant_players()`를 추가해 MA가 알고 있는 모든 플레이어(집 전체, 방 무관)를 노출하고, 카드 헤더에 선택 목록(2대 이상일 때만 표시, 방에서 자동으로 찾은 스피커가 기본 선택)을 추가 -- 플레이리스트를 누르면 그 순간 선택된 스피커로 재생됨
- **부수 발견/수정**: 이 작업 중 기기 제어 카드의 on/off 토글이 media_player에서는 항상 꺼짐으로 표시되는 버그 발견 -- `deviceCardToggleHTML()`의 `isOn` 판정이 `state === 'on'`만 체크했는데, media_player는 켜져 있어도 상태값이 "playing"/"idle"/"paused" 등으로 나오지 "on"이 아님(HA의 다른 도메인과 다른 상태값 체계). media_player 도메인일 때는 "off"/"unavailable"/"unknown"이 아니면 켜진 것으로 판정하도록 수정 -- 스피커/TV 카드 전반에 영향
- 실기기로 재검증: 플레이리스트 9개 노출, 목록이 컴팩트하게 스크롤, 스피커 선택 목록에 집 안 모든 MA 플레이어가 뜨는 것, 다른 스피커로 바꾼 뒤 재생하면 그쪽으로 나가는 것, 재생 중일 때 토글이 켜짐으로 표시되는 것까지 확인

## 1.1.0-beta.113

### 수정 (Fix) — beta.112의 플레이리스트 선택이 실제로는 재생되지 않던 문제
- **재현**: beta.112 배포 직후 실기기로 클릭까지 검증하다 발견 -- 카드는 정상 표시됐지만 플레이리스트를 선택해도 스피커가 계속 `idle`로 멈춰있음
- **원인**: `music_assistant.play_media`의 대상으로 방-이름 매칭에 쓰던 "안방 스피커"(`media_player.bed_speaker`, `platform: cast`)를 그대로 넘겼는데, Music Assistant는 같은 물리 스피커라도 자기 것으로 등록할 때 **완전히 별도의 엔티티**(`media_player.ma_bed_speaker`, `platform: music_assistant`)를 새로 만듦 -- 이름 패턴으로도 연결이 안 되고(실기기 확인: "거실 스피커" ↔ MA 쪽은 "ma living speaker") 오직 area(구역)만 공유. REST 서비스 호출 자체는 성공(`{"success": true}`)으로 응답해서 오류 없이 조용히 아무 일도 안 일어났음. 애초에 "MA가 관리하는 스피커인지" 판별에 썼던 `supported_features & 512` 비트도 근본적으로 틀린 신호였음 -- 512는 HA 표준 `PLAY_MEDIA` 기능 비트라 캐스트 등 거의 모든 미디어 플레이어가 가지고 있어서 MA 전용 신호가 될 수 없었음
- **수정**: `core/ha_registry.py`가 기존 숨김기기 조회에 쓰던 WebSocket 세션에 `config/device_registry/list`도 같이 조회하도록 확장해 엔티티→디바이스→구역(area_id)까지 캐시하고, 새 `find_music_assistant_sibling(entity_id)`로 "이 엔티티와 같은 구역에 있는 `platform: music_assistant` 미디어 플레이어"를 찾아 반환. `_h_media()`가 이제 이 함수로 찾은 실제 MA 엔티티 id를 카드/재생 대상으로 쓰고(표시 이름은 그대로 "안방 스피커" 등 방 이름 유지), sibling을 못 찾으면 기존처럼 단순 전원 on으로 대체
- 배포 직후 실기기로 클릭까지 검증하는 과정에서 이 수정 자체가 또 다른 문제(같은 구역에 MA 기기가 여러 대일 때 엉뚱한 걸 고름)를 드러냄 -- beta.114 참고

## 1.1.0-beta.114

### 수정 (Fix) — beta.113의 구역 매칭이 한 방에 MA 기기가 여러 대면 엉뚱한 걸 고르던 문제
- **재현**: beta.113 배포 후 실기기로 "안방에 음악 틀어줘" 클릭까지 재검증하다 발견 -- 카드의 `entity_id`가 `media_player.ma_bed_speaker`가 아니라 `media_player.ma_bed_nesthub`로 나옴. 실제로 안방에는 MA 기기가 "ma bed nesthub"(Nest Hub)/"ma bed speaker"(Chromecast Audio)/"ma bed group"(Cast Group)/"ma bed googlehome"(Google Home Mini) 4대나 있고, beta.113의 구역(area)만 보는 매칭이 WebSocket 응답 순서상 처음 걸리는 걸 그냥 골랐음
- **수정**: `config/device_registry/list`에서 `model`도 같이 캐시해서, 구역 일치 후보가 둘 이상이면 원래 엔티티(`media_player.bed_speaker`, model="Chromecast Audio")와 **같은 model**인 후보로 좁힘 -- MA 쪽 기기 이름은 사용자가 MA에 추가할 때 임의로 붙인 거라 원본과 안 겹치지만(예: "거실 스피커" ↔ "ma living speaker"), model은 물리적으로 동일 기기라 항상 일치함. 그래도 구역+model로도 못 좁히면(모델까지 같은 게 여러 대 등) 추측하지 않고 그냥 기존 단순 전원 on으로 대체 -- 이 세션 내내 지켜온 "여러 개면 묻거나 포기, 추측 안 함" 원칙 그대로 적용
- 실기기로 재검증: "안방에 음악 틀어줘" → 카드에 `media_player.ma_bed_speaker` 정확히 노출까지는 확인. 플레이리스트 클릭 자체가 또 다른 문제(타임아웃)로 막혀 있던 것을 이어서 beta.115에서 발견/수정

## 1.1.0-beta.115

### 수정 (Fix) — beta.114까지 고쳤는데도 플레이리스트 클릭이 "재생 시작"으로 안 이어지던 문제
- **재현**: beta.114로 정확한 MA 엔티티(`media_player.ma_bed_speaker`)를 타겟팅하게 된 뒤 실제로 카드에서 클릭까지 재현 -- `/api/device/control`이 `{"success": false}`를 반환. 그런데 MCP로 완전히 동일한 `music_assistant.play_media(entity_id=media_player.ma_bed_speaker, media_id=..., media_type=playlist)`를 직접 호출하면 몇 초 뒤 정상적으로 `playing` 상태(실제 곡 "그대만 있다면" 재생)까지 확인됨 -- 서비스 자체와 엔티티 매핑은 이미 맞았음
- **원인**: `music_assistant.play_media`의 REST 서비스 호출은 실제 Cast 기기와의 재생 핸드셰이크가 끝날 때까지 응답하지 않는데(실측 5초 안팎), `core/ha_client.py`의 `ha_call_service_api()`가 모든 서비스 호출에 일괄 적용하던 타임아웃이 3초뿐이라 매번 타임아웃으로 실패 처리됐음 -- 다만 HA 쪽에서는 호출이 이미 접수돼 그대로 진행되니, 사용자 입장에선 "실패라고 뜨는데 몇 초 뒤 보면 재생되고 있는" 혼란스러운 상태였음
- **수정**: `ha_call_service_api()`에 `timeout` 파라미터 추가(기본값 3초, 기존 모든 호출부는 그대로 3초 유지 -- 조명/팬/스위치 등 즉시 응답하는 호출의 체감 속도에 영향 없음). `antigravity_api.py`의 `/api/device/control`이 `domain == "music_assistant"`일 때만 12초로 늘려서 호출 -- 이 엔드포인트는 카드 클릭 한 번에 대한 별도 요청이라 자연어 대시보드의 "0.05초" 응답 경로와는 무관하고, `ThreadingHTTPServer`라 이 요청이 오래 걸려도 다른 요청을 막지 않음
- 실기기로 최종 재검증: "안방에 음악 틀어줘" → 카드에서 "Favorite Songs" 클릭 → `/api/device/control`이 `{"success": true}` 반환, `media_player.ma_bed_speaker`가 실제로 `playing`(곡: 그대만 있다면 - Nerd Connection)으로 전환되는 것까지 전체 흐름 확인

## 1.1.0-beta.112

### 추가 (Feature) — "OO에 음악 틀어줘"에 Music Assistant 최근 플레이리스트 선택 카드 지원
- **배경**: 지난 턴에서 "안방에 음악 틀어줘"가 (1) "음악"이라는 단어 자체를 인식 못 하고, (2) "스피커"를 명시해도 단순 전원 on/off만 호출된다는 두 가지 문제를 확인했음. 이 집 HA에 Music Assistant 애드온이 이미 설치·실행 중인 것을 확인해, 실제 재생까지 되는 기능으로 구현 (엔티티 매핑/타임아웃 관련 후속 수정은 beta.113~115 참고)
- **신규 파일 `core/music_assistant.py`**: `get_config_entry_id()`(REST API엔 config entry 조회 엔드포인트가 없어서, `core/ha_registry.py`가 숨김 기기 조회에 쓰던 것과 같은 Supervisor Core WebSocket 프록시로 `config_entries/get` 호출, 60초 캐시) / `get_recent_playlists()`(`music_assistant.get_library`를 `?return_response`로 호출해 `order_by=last_played_desc` 최근 플레이리스트 최대 6개, 이름·이미지·URI 반환)
- **`core/ha_client.py`**: "음악"/"노래"/"플레이리스트"를 스피커 트리거 단어로 추가(기존엔 "스피커"만 반응). 켜기 명령이 이 단어들과 함께 오고, 방 안에 스피커가 정확히 하나로 좁혀지고, 그 스피커가 실제 MA 관리 기기이고, 플레이리스트가 하나 이상 있으면 -- 그냥 켜는 대신 그 스피커를 켜고 새 스레드-로컬 `_playlist_card_sink`에 플레이리스트 목록을 담아 "🎵 OO에서 재생할 플레이리스트를 선택해 주세요" 반환. 셋 중 하나라도 안 맞으면(방 미지정/여러 대, MA 미관리 스피커, 라이브러리 없음) 기존처럼 단순 전원 on 문구로 자연스럽게 대체. `ALLOWED_CARD_SERVICES`에 `music_assistant: {play_media}` 추가 -- 카드에서 플레이리스트 클릭 시 기존 `/api/device/control` 엔드포인트를 그대로 재사용해 실제 재생 호출
- **`core/streamer.py`**: `device_card`와 별개로 새 SSE 이벤트 `playlist_card` 추가 -- 이번 턴에 실제로 제어된 게 없어도(플레이리스트는 아직 "선택 대기" 상태일 뿐) 별도 사이드채널로 전송
- **`core/ui/scripts.py`/`core/ui/styles.py`**: `.answer-content` 아래 `.device-card-list`와 별개로 `.playlist-card-list`를 추가해 썸네일+이름의 클릭 가능한 그리드로 렌더링(`renderPlaylistCard`/`handlePlaylistSelect`), 클릭 시 `music_assistant.play_media(media_id=선택한 URI, media_type=playlist)`를 그 스피커 엔티티로 직접 호출하고 선택된 항목에 표시 강조
- 실제 Music Assistant 라이브러리에서 최근 플레이리스트("오늘의 TOP 100: 대한민국", "Favorite Songs" 등) 조회 확인, mock 데이터로 MA 미관리 스피커/빈 라이브러리 시 기존 단순 on 동작으로 정상 대체(회귀 없음) 확인. 대화 새로고침 시 카드 복원은 이번 범위에서 의도적으로 제외(장치 제어 카드처럼 텍스트만 남고 선택 목록은 다시 안 뜸)

## 1.1.0-beta.111

### 수정 (Fix) — "OO이 집에 있어?" 같은 특정 가족 구성원 재실 질문을 못 알아듣던 문제
- **재현**: "다은이 집에 있어?"/"다은 집에 있어?"/"다은 왔어?" 전부 재실 관련 키워드("재실"/"누구있어"/"누가있어"/"집에누가"/"어디있어"/"어디야")에 하나도 안 걸려서 "무슨 말씀인지 파악하지 못했습니다" 재질문으로 빠짐. `get_presence_summary()`는 애초에 프롬프트에서 특정 사람 이름을 찾아 "OO 님은 지금 집에 있어요" 형태로 답하는 로직이 이미 있었는데, 그 함수로 라우팅하는 트리거 자체가 "이름 없이 묻는" 문구만 다루고 있었음
- **수정**: `person.*` 엔티티의 실제 friendly_name이 프롬프트에 들어있고 "집에"/"왔어"/"귀가"/"외출"/"나갔" 중 하나가 같이 있으면 `get_presence_summary()`로 라우팅하도록 branch 14 확장. 이름 없이 그냥 "OO 있어?"(예: "다은이 있어?")는 일부러 그대로 둠 -- "다은 생일 있어?"(branch 16, 기념일) 같은 무관한 질문과 겹치는 걸 막기 위해 "있어" 단독은 트리거로 안 씀. "집에있"/"집에누구" 두 문구도 트리거 목록에 추가해 "집에 있어?"/"집에 누구 있어?"(어순이 다른 "집에 누가 있어?" 변형)도 커버
- **부수 발견/수정**: 같은 테스트 중 "집에 누구 있어?"가 재실 응답이 아니라 엉뚱한 인사말("안녕하세요! ... 무엇을 도와드릴까요?")로 나가는 것 발견 -- 인사 분기가 "누구"를 단독 키워드로 걸어놔서 "너 누구야?"뿐 아니라 "집에 누구 있어?"의 "누구"에도 반응해 재실 분기(14)보다 먼저 가로챘음. "누구"를 "누구야"/"누구세요"로 좁혀서 "누구야?" 인사 인식은 그대로 두고 재실 질문과의 충돌만 제거
- 실제 `person.daeun`("다은") 데이터로 "다은이 집에 있어?"/"다은 왔어?"/"다솔 왔어?"/"집에 누구 있어?"/"집에 누가 있어?"/"누가 집에 있어?" 모두 올바른 재실 답변 확인, "다은 생일 있어?"(기념일 분기)/"너 누구야?"(인사)/"다은이 있어?"(의도적으로 미해결) 회귀 없음 재검증

## 1.1.0-beta.110

### 수정 (Fix) — 기기 제어 카드에서 켜기 전엔 없던 밝기/색온도 등 조절 컨트롤이 켜도 안 나타나던 문제
- **재현**: "안방 책상 등 꺼" 실행 시 카드에 on/off 스위치만 나옴(정상 -- 꺼진 조명은 `light.mi_stand_light`가 `brightness`/`color_temp_kelvin`을 실제로 `null`로 보고함). 그런데 카드의 on/off 스위치를 눌러 다시 켜도 슬라이더가 계속 안 나타남 -- 카드 자체를 다시 그리지 않았기 때문
- **원인**: `core/ui/scripts.py`의 `handleDeviceToggle()`이 `/api/device/control`로 제어 요청만 보내고 끝 -- 성공 후에도 카드 DOM을 다시 그리지 않아, 실제로는 켜져서 밝기/색온도 값이 생겼어도 화면엔 반영 안 됨. `build_device_cards()`가 애초에 값이 `None`인 필드는 카드에 아예 안 담는 것(꺼진 상태의 정상 동작)과 별개의, 순전히 프론트엔드 갱신 누락 문제
- **수정**: 이미 대화 복원용으로 있던 `GET /api/device/state?entity_id=X`를 재사용해 `handleDeviceToggle()`이 제어 요청 완료 후(실제 기기가 새 상태를 HA에 반영할 시간을 위해 400ms 대기) 그 엔티티 하나만 다시 조회해 카드 DOM을 새로 그리는 `refreshDeviceCard()` 추가 -- 온/오프 토글은 라이트/팬/커튼/climate/media_player 5개 도메인 카드가 전부 공유하는 하나의 함수라 도메인별 추가 코드 없이 전체에 동일하게 적용됨
- 라이트(밝기/색온도), climate(목표온도/모드), media_player(볼륨)처럼 꺼졌을 때 값이 `null`이 되는 도메인은 켜는 순간 실제 값이 나타나고, 팬/커튼처럼 꺼져도 값을 유지하는 도메인은 큰 차이 없이 항상 최신 상태로 갱신됨 -- 어느 쪽이든 화면에 보이는 값이 항상 실제 기기 상태와 일치

## 1.1.0-beta.109

### 수정 (Fix) — 한 문장에 여러 기기 종류 + 동사 하나만 있으면 첫 번째 기기만 제어되던 문제
- **재현**: "안방 등하고 선풍기 켜" / "안방 등 선풍기 켜" 둘 다 선풍기만 켜지고 등은 조용히 무시됨. "안방 등 켜고 커튼 닫아"처럼 동사가 절마다 반복되는 압축 명령("-고")은 이미 지원했지만, "등하고 선풍기 켜"처럼 명사 여러 개가 동사 하나를 공유하는 문장은 애초에 절로 안 쪼개짐
- **원인**: `_execute_single_control_clause()`가 커튼→선풍기→조명→가습기/제습기→위험기기 스위치→에어컨→미디어 순서로 `if 키워드매치: ... return` 블록을 검사하는데, 하나라도 서비스 호출까지 성공하면 그 자리에서 함수 전체가 즉시 `return`되어 뒤에 있는 다른 기기 종류 블록은 아예 실행되지 않았음
- **수정**: 7개 분기를 각각 중첩 함수(`_h_curtain`/`_h_fan`/`_h_light`/`_h_humidifier`/`_h_appliance`/`_h_climate`/`_h_media`)로 감싸 "이 종류는 언급 안 됐음/실행할 동사 없음"이면 `None`, 실행(성공 또는 개별 오류)했으면 메시지 문자열을 반환하도록 바꾸고, 바깥에서 7개를 전부 호출해 `None`이 아닌 결과를 전부 모아 줄바꿈으로 이어붙임 -- 기기 종류가 하나만 언급된 기존 명령은 결과가 완전히 동일(그 종류 하나만 논-None을 반환하므로)
- 퍼센트(밝기/풍량) 서브분기, 방 미지정 시 되묻기, 숨김 기기 확인, 위험 스위치(보일러 등) 끄기 확인 등 각 분기 내부 로직은 그대로 -- 이제 "안방 등 보일러 꺼"처럼 즉시 실행되는 기기와 확인이 필요한 기기가 한 문장에 섞여도 즉시 실행할 건 실행하고 확인 질문은 별도로 붙여서 응답
- 실제 mock 데이터로 "안방 등하고 선풍기 켜"/"안방 등 선풍기 켜"가 둘 다 켜지는 것 확인, 기존 "안방 등 켜"(단일 기기)/"안방 등 켜고 커튼 닫아"(압축 명령)/"안방 선풍기 켜고 에어컨 꺼"(반대 동사) 전부 회귀 없음 재검증

## 1.1.0-beta.108

### 수정 (Fix) — 응답 텍스트의 목록이 마크다운 리스트로 렌더링되지 않던 문제
- 지금까지 대부분의 요약/상태 응답이 목록 항목 앞에 "• "(가운뎃점) 문자를 써왔는데, 실제 프론트엔드가 쓰는 마크다운 파서(`core/markdown_parser.py`에 번들된 `marked`)는 표준 "- "/"* "/"+ " 문법만 리스트로 인식함 -- 이미 `get_comprehensive_home_summary()`만 "- "를 쓰고 있었고 나머지는 전부 "• "라 UI에 리스트가 아닌 밋밋한 텍스트로 보였음
- `core/ha_client.py`(2건), `core/ha_engine.py`(10건), `core/renderers.py`(19건), `core/sensors.py`(30건), `core/system_info.py`(2건) 총 63건의 "• "를 "- "로 일괄 치환 -- 중첩 하위 목록("  - ")이나 한 문자열 안에 여러 항목이 join되는 케이스(조명 목록, 자동화 목록 등)도 개별 확인
- "안방 등 목록", "뭐 할 수 있어", "우리집 상황 어때" 등 대표 응답 각각 재생성해 전부 "- " 마크다운 리스트로 나오는 것 확인, 회귀 없음

## 1.1.0-beta.107

### 추가 (Feature) — "안방 등 목록/리스트/뭐있어/보여줘" 같은 목록 조회 지원
- 기존에 "안방 등 켜져있어?" 같은 상태 질문에만 반응하던 `is_status_query()`(→ `get_device_status_answer()`, 방/도메인에 맞는 기기를 전부 개별 상태와 함께 나열하는 함수가 이미 있었음)가 "목록"/"리스트"/"뭐있어"/"뭐가있어"/"보여줘" 문구는 인식하지 못해 아무 분기에도 안 걸리고 beta.106의 "무슨 말씀인지 파악 못함" 재질문(그 전엔 종합 브리핑)으로 빠졌음
- `_STATE_QUERY_PATTERNS`에 이 문구들을 추가 -- 기존 상태-질문 목록 함수를 그대로 재사용하므로 새 코드 경로 없이 동일한 출력(방 지정 시 그 방만, 미지정 시 집 전체, 숨김/진단용 LED는 기존처럼 제외)
- "안방 등 목록"/"안방 조명 리스트"/"조명 뭐있어"(방 생략 시 집 전체)/"안방 등 보여줘" 전부 검증, 기존 제어 명령·상태 질문 회귀 없음 확인

## 1.1.0-beta.106

### 수정 (Fix) — 아무 키워드에도 안 걸리는 완전히 무관한 말도 종합 상황 브리핑이 나가던 문제
- `handle_agent_chat()`의 branch 1~17 중 아무것도 매치되지 않으면(방/기기/자동화/날씨/상태 키워드도, 제어 동사도 전혀 없음) 맨 끝 "Fallback"이 무조건 `get_comprehensive_home_summary()`(🏠 우리집 스마트홈 종합 상황 브리핑)를 반환하고 있었음 -- "오늘 점심 뭐 먹지" 같은 완전히 무관한 말에도 종합 브리핑이 나가는 등, 이번 세션 내내 고쳐온 "모르면 추측하지 말고 되묻기" 원칙에 어긋남
- Fallback을 "무슨 말씀인지 정확히 파악하지 못했습니다..." 재질문으로 교체. branch 17(진짜 "상태/상황/우리집/브리핑" 요청)은 그대로 종합 브리핑을 반환하도록 유지 -- 이건 사용자가 실제로 그걸 요청한 경우이므로 손대지 않음
- 제어 명령/진짜 브리핑 요청/무관한 질문/인사 4가지 케이스 모두 회귀 없이 재검증

## 1.1.0-beta.105

### 추가 (Feature) — 기기 제어 카드에 climate 모드/팬 속도 선택 UI 추가
- climate 카드가 목표온도 슬라이더만 있었음 -- `hvac_modes`(냉방/난방/제습/송풍 등)와 `fan_modes`(약/중/강 등)는 이미 카드 데이터에 있었는데 프론트엔드가 안 그리고 있었음
- `build_device_cards()`에 `fan_mode`/`fan_modes` 필드 추가(hvac_modes는 기존부터 있었음 -- climate 엔티티의 `state` 자체가 곧 현재 hvac_mode라 별도 "현재 모드" 필드는 불필요). `core/ui/scripts.py`에 드롭다운 행 렌더러(`deviceCardRowSelect`) 신설 -- 숫자 범위가 아닌 이름 있는 모드 목록이라 슬라이더 대신 `<select>` 사용, 현재 값이 자동 선택됨
- 변경 시 `climate.set_hvac_mode`/`climate.set_fan_mode`를 직접 호출 -- `ALLOWED_CARD_SERVICES`에 `set_fan_mode` 추가
- 실제 "안방 화장실 환풍기 Climate"(beta.104에서 다룬 기기) 데이터로 카드 스키마 검증 -- 모드 5개, 팬속도 4개 옵션 모두 정상 노출

## 1.1.0-beta.104

### 수정 (Fix) — climate 도메인으로 노출된 환풍기를 "제어할 수 있는 선풍기/환풍기가 없습니다"로 못 찾던 문제
- **재현**: "안방 화장실 환풍기 켜"가 실패 -- 실제로는 `climate.bed_toilet_himpel_fan_climate`("안방 화장실 환풍기 Climate")라는 기기가 존재하는데, 선풍기/환풍기 분기가 `fan.*` 도메인만 검색해서 이 himpel 욕실 환풍기(온습도 기반 모드까지 지원해 HA에서 climate 도메인으로 노출됨)를 완전히 놓치고 있었음
- **수정**: `hvac_modes`에 `fan_only`가 있는 climate 엔티티 중 이름에 "팬/선풍기/환풍기/실링팬"이 포함된 것도 후보에 포함하도록 확장. 실제 서비스 호출은 도메인별로 분기 -- `fan.*`는 기존대로 `fan.turn_on/off`, climate로 노출된 환풍기는 `climate.set_hvac_mode`(켜기=`fan_only`, 끄기=`off`)를 사용 -- 단순 `climate.turn_on`은 마지막 hvac_mode(난방/냉방/제습 등)로 복귀할 수 있어 "환풍기를 켠다"는 의도와 다를 수 있기 때문
- 응답 문구에서 기기 이름의 불필요한 "Climate" 접미사도 제거(`_strip_device_type_suffix` 재사용) -- "안방 화장실 환풍기 Climate 가동을 켰습니다" 대신 "안방 화장실 환풍기 가동을 켰습니다"
- 실제 데이터(`climate.bed_toilet_himpel_fan_climate` + 별개의 `fan.toilet_fan`, 방 이름 없는 동명이인)로 검증 -- "안방 화장실 환풍기"는 정확히 안방 것만 골라 `set_hvac_mode`로 제어, 기존 순수 `fan.*` 기기는 회귀 없음

## 1.1.0-beta.103

### 추가 (Feature) — 예약 실행 개수/목록을 HA 센서로도 노출
- `/api/status`에 `scheduled_count`(대기 중인 예약 실행 개수)와 `scheduled_list`(각 항목의 남은 시간/설명) 필드 추가 -- 기존에 대화로만("예약 실행 목록 보여줘") 확인 가능했던 정보를 HA 엔티티로도 노출
- 커스텀 컴포넌트(`ha-antigravity-cli`)에 `sensor.antigravity_cli_scheduled_count` 신설 -- 상태값은 개수, `scheduled_list` 속성에 목록 포함. 기존 상태 폴링(coordinator)이 이미 `/api/status`를 주기적으로 읽고 있어서 별도 요청 경로 없이 그대로 값을 얹음

## 1.1.0-beta.102

### 수정 (Fix) — 맨몸 후속 명령("켜"/"꺼")에는 기기 제어 카드가 안 뜨던 문제
- beta.90의 맨몸 후속 명령 처리("닫아"만 말해도 직전에 언급한 커튼을 닫음)는 `resolve_control_scope()`를 거치지 않고 직접 `ha_call_service_api`를 호출해서, beta.99의 기기 제어 카드가 이 경로에서는 안 뜨고 있었음
- 이 경로도 카드 캡처 싱크에 동일하게 적재하도록 수정 -- 라이브 시나리오("안방 선풍기 꺼" → "켜" → "책상등 꺼" → "켜")에서 매 단계 카드가 정상적으로 함께 나오는 것까지 확인

## 1.1.0-beta.101

### 추가 (Feature) — 방 이름 생략 시 마지막 언급한 방을 기억해서 재사용
- **재현**: "안방 선풍기 꺼" → "켜"(같은 선풍기 재실행, beta.90에서 이미 동작)까지는 괜찮았지만, 이어서 방 이름 없이 "책상등 꺼"라고 하면 "책상 등"이 안방/작은방 두 곳에 있어 매번 "어느 방이요?"라고 재질문했음 -- 직전에 안방 얘기를 했다는 맥락을 전혀 활용하지 못했음
- **수정**: `resolve_control_scope()`가 방이 여러 곳에서 후보로 나와 모호할 때, 재질문 전에 `get_last_room_context()`(기존에 "습도는?" 같은 방 생략 센서 질문에만 쓰이던 대화별 "마지막 언급 방" 기억)를 먼저 확인 -- 그 방이 후보 중 하나면 곧장 그 방으로 좁혀서 처리. 방을 명시하거나 이렇게 기억으로 좁힌 경우 모두 `set_last_room_context()`로 다시 기록되므로 "책상등 꺼" 이후 "켜"는 방금 끈 안방 책상 등을 그대로 다시 켬(beta.90 맨몸 후속 명령 기능과 자연 연동)
- 기존 로직(방 언급 시 그 방, "전체"/"다" 명시 시 전체, 후보가 애초에 한 방뿐이면 그대로 처리)은 전혀 안 바뀜 -- 정말 여러 방에 걸쳐 모호하고 기억된 방도 후보에 없을 때만 재질문
- 부가 효과: 기존에 센서 질문 전용이던 "마지막 언급 방" 기억을 기기 제어와 공유하게 되어, "안방 선풍기 꺼" 다음에 "습도는?"이라고 물어도 이제 안방 습도로 정확히 답함(이전엔 반대 방향만 됐음)
- 실제 데이터 형태(안방 선풍기 Fan, 안방/작은방 책상 등 각 2개)로 4단계 시나리오 전체 재현·검증 완료

## 1.1.0-beta.100

### 수정 (Fix) — 색온도 전용 조명에 색상 조절 UI가 잘못 표시되던 문제
- "안방 책상 등"(`supported_color_modes: ["color_temp"]`, RGB 미지원)에서 색상 피커가 나타남 -- HA Core는 color_temp 전용 조명에도 현재 색온도를 근사 변환한 `rgb_color`/`hs_color`/`xy_color` 값을 항상 같이 보고하는데(예: 3895K일 때 `rgb_color: [255,203,161]`), `build_device_cards()`가 이 속성의 존재 여부만으로 "색상 조절 가능"을 판단해서 실제로는 못 바꾸는 색상 조절 UI가 나왔음
- `supported_color_modes`(밝기/색온도/색상 각각을 이 필드로만 판단)를 확인하도록 수정 -- `rgb`/`rgb_white`/`rgbw`/`rgbww`/`hs`/`xy` 중 하나가 실제로 있을 때만 색상 피커 표시. 색온도 범위(`min/max_color_temp_kelvin`)는 원래부터 각 기기가 보고하는 실제 값을 그대로 썼던 것이라 이번 수정과 무관하게 이미 정확했음 (실기기 "안방 책상 등"으로 확인: 2700~5000K)
- 실제 색온도 전용 조명과 실제 RGB 지원 조명(안방 다운라이트) 둘 다로 검증: 전자는 색상 피커 없이 밝기+색온도만, 후자는 밝기+색온도+색상 전부 정상 표시

## 1.1.0-beta.99

### 추가 (Feature) — 기기 제어 카드 (인터랙티브 UI)
- 지금까지 기기 제어 결과는 항상 텍스트 한 줄("💡 안방 등 조명을 켰습니다.")뿐이었음 -- 이제 그 아래에 실제로 눌러서 조작할 수 있는 카드가 같이 표시됨: 모든 도메인 공통 on/off 토글, 조명은 밝기·색온도·색상, 선풍기는 풍량, 에어컨은 목표온도, 커튼은 위치, 미디어 플레이어는 볼륨
- `core/ha_client.py`에 스레드-로컬 `_card_entity_sink` 신설 -- `resolve_control_scope()`가 성공적으로 확정한 대상 엔티티를 이번 턴 한정으로 포착(`get_last_action_entities()`, "가져가면 비워지는" drain 방식이라 날씨 질문처럼 아예 기기 제어를 안 한 턴이 이전 턴의 기록을 잘못 물려받지 않음). 예약 실행이 아직 안 걸린 명령이나 위험/숨김 기기 확인 대기 중에는 자연히 비어 있어 카드가 안 뜸
- `build_device_cards()`가 이미 갖고 있던 HA raw attributes(brightness, color_temp_kelvin, rgb_color, percentage, current_position, volume_level 등)에서 도메인별로 필요한 값만 뽑아 카드 JSON으로 변환 -- 새 HA 호출 없음
- `core/streamer.py`의 `stream_fast_dashboard()`가 `text` 이벤트 다음에 새 `device_card` SSE 이벤트로 전송. 프론트(`core/ui/scripts.py`)는 도메인별 슬라이더/토글을 그리고, 조작 시 새 `POST /api/device/control`을 직접 호출 -- 이 경로는 챗 파이프라인을 거치지 않으므로 `ALLOWED_CARD_SERVICES`(도메인별 허용 서비스 화이트리스트)로 별도 방어
- 대화 복원 시엔 엔티티 id만 저장해뒀다가 새 `GET /api/device/state`로 최신 상태를 다시 받아와 카드를 그림 -- 그 사이 다른 경로로 상태가 바뀌었어도 항상 최신값 표시

## 1.1.0-beta.98

### 수정 (Fix) — 브라우저 아닌 클라이언트에서 /api/chat 응답이 끝나지 않고 계속 걸려있던 문제
- SSE 응답 헤더가 `Connection: keep-alive`였는데, SSE 바디는 `Content-Length`도 `Transfer-Encoding: chunked`도 안 쓰기 때문에 브라우저의 `fetch()`/`EventSource`는 문제없이 청크를 계속 읽어들이지만, 완료된 응답을 기다리는 다른 클라이언트(aiohttp, curl, requests 등)는 서버가 실제로 다 보냈는데도 연결이 끊기지 않아 자체 타임아웃까지 계속 대기했음
- `Connection: close`로 변경 -- 응답 전송이 끝나면 연결도 같이 끊겨서 비-브라우저 클라이언트도 정상적으로 완료를 인지함

## 1.1.0-beta.97

### 추가 (Feature) — 예약(지연) 실행 개수/목록 조회
- beta.94에서 추가된 "N분 뒤에 꺼줘" 예약 실행은 걸어놓은 뒤 확인할 방법이 없었음 -- `core/ha_client.py`에 대기 중인 타이머를 추적하는 레지스트리(`_scheduled_controls`, 스레드-세이프)를 추가해 `_schedule_delayed_control()`이 예약을 걸 때 등록하고 실제 실행되면 제거
- "예약 실행 목록 보여줘", "예약 몇 개야?" 같은 질문을 `is_scheduled_controls_query()`로 판별해 `get_scheduled_controls_answer()`가 남은 시간 순으로 정렬된 개수/목록을 응답 (예: "⏰ 예약된 실행 2건\n• 29초 후: 거실 커튼을 닫겠습니다\n• 2분 4초 후: 안방 등을 끄겠습니다")
- `core/ha_engine.py`의 의도 분기에서 이 질문을 기존 제어 동사("켜"/"꺼"/"실행"/...) 분기보다 먼저 체크 -- "예약 실행 목록"이라는 문구 자체에 "실행"이 들어있어서, 순서가 바뀌면 제어 분기가 먼저 가로채 "무엇을 켜거나 끄시려는 건지 확인하지 못했습니다" 오답이 나갔을 것
- 예약과 마찬가지로 이 레지스트리도 애드온 프로세스 메모리에만 존재 -- 애드온이 재시작되면 목록도 함께 비워짐 (beta.94와 동일한 제약)

## 1.1.0-beta.96

### 추가 (Feature) — 숨김 처리된 기기는 자동 제외하거나 확인 후에만 제어
- **배경(2026-09-05 사고)**: HA에서 "숨김(hidden)" 처리해둔 "거실 플러그"(실제로는 집 서버 전원)가 방 범위 제어 명령의 후보 목록에 그대로 포함돼 실제로 꺼졌고, 서버가 죽으면서 HA 자체가 20분 넘게 응답 불가 상태가 됨. hidden 여부는 REST(`/api/states`)에 없고 엔티티 레지스트리(WebSocket 전용)에만 있어서 기존 코드는 이걸 전혀 몰랐음
- **수정**: `websocket-client` 라이브러리 추가(Dockerfile) + 신설 `core/ha_registry.py`가 Supervisor의 Core WebSocket 프록시(`ws://supervisor/core/websocket`)로 `config/entity_registry/list`를 조회해 숨김 엔티티 id 집합을 60초 캐시로 유지 (매 명령마다 조회하면 고속 디스패치 취지가 무색해지므로 캐시)
- `resolve_control_scope()`가 후보를 확정하는 모든 경로(특정 방/전체/모호)에서 이 집합을 적용: **숨김 기기 외에 다른(안 숨겨진) 후보가 있으면 숨김 기기는 조용히 제외**, **숨김 기기가 유일한 후보면 실행하지 않고 "숨김 처리된 기기인데 그래도 제어할까요?" 확인**을 받은 뒤에만 실행 (beta.95의 위험 가전 확인과 동일한 `pending_confirmation` 메커니즘 재사용 -- 도메인/서비스/추가 데이터(퍼센트 등)까지 일반화해서 켜기/끄기/열기/닫기/밝기·풍량 설정 전부 지원)
- websocket-client 설치가 실패하거나 HA WebSocket에 연결할 수 없으면 숨김 필터링이 조용히 비활성화(빈 집합)될 뿐 기기 제어 자체는 계속 정상 동작 -- 안전 기능 추가가 새로운 실패 지점이 되지 않도록 방어
- 실제 "거실 플러그"(숨김, `hidden_by: "user"`)로 라이브 재현·검증: 이제 "거실 플러그 꺼줘"는 즉시 실행되지 않고 확인을 먼저 요구함

## 1.1.0-beta.95

### 추가 (Feature) — 보일러/히터/콘센트 등 끄기는 경고 후 재확인 받도록 변경
- 기존에 이미 커스텐 목록으로 구분해두었던 위험 가전 스위치(`APPLIANCE_SWITCH_KEYWORDS`: 보일러/히터/전기스토브/콘센트/플러그)를 끌 때, 이제 즉시 실행하지 않고 "⚠️ OO를 정말 끄시겠어요?" 경고를 먼저 응답하고 실제 종료는 보류
- 다음 턴에 "응"/"네"/"어"/"그래" 등 짧은 긍정으로 답하면 그제서야 실제로 끄고, "아니"/"취소" 등 부정이면 취소, 그 외(전혀 다른 새 명령 등)는 보류 중이던 확인을 조용히 버리고 새 명령을 그대로 처리 -- `core/session_manager.py`에 `set_pending_confirmation`/`get_pending_confirmation`/`clear_pending_confirmation` 신설, `core/ha_engine.py`의 `handle_agent_chat()` 맨 앞(0단계)에서 확인 답변인지 먼저 판별
- 켜기(turn_on)는 게이트 없음 -- 위험은 "의도치 않게 꺼짐"에 있지 예상치 못하게 켜지는 것에는 없다고 보아 켜기만 즉시 실행 유지
- **부가 발견**: 테스트 중 `resolve_control_scope()`가 방 이름이 전혀 없는 단일 후보 기기를 "찾지 못했습니다"로 오판하는 별개의 잠재 버그 발견 -- 현재 실제 집 데이터로는 재현 안 됨(보일러 관련 스위치가 전부 방 이름 포함)이라 이번 릴리스 범위 밖으로 분리해 별도 트래킹

## 1.1.0-beta.94

### 추가 (Feature) — "안방 등 10분 뒤에 꺼줘" 같은 예약(지연) 실행 지원
- "N초/분/시간 뒤/후(에)" 및 "N초/분/시간 있다가/있으면/있다" 표현을 인식하는 `_extract_delay()` 추가. 매치되면 지연 문구를 제거한 나머지 명령을 기존 로직(단일/복합 명령 모두)으로 그대로 해석하되, 실제 HA 서비스 호출은 즉시 실행하지 않고 `threading.Timer`로 예약
- 구현 방식: `ha_call_service_api()`가 스레드-로컬 "캡처 모드"를 지원하도록 변경 -- 예약 명령을 해석하는 동안에는 실제 네트워크 호출 대신 (도메인, 서비스, 데이터) 튜플만 수집하고, 지연 시간이 지난 뒤 백그라운드 타이머가 그 호출들을 그대로 재생. 방/기기가 모호해서 재질문이 나오는 경우엔 아무것도 캡처되지 않으므로 예약 자체가 걸리지 않고 재질문을 그대로 응답
- **주의(제약)**: 예약은 애드온 프로세스 메모리에만 존재 -- 대기 중 애드온이 재시작/리빌드되면 예약된 실행은 사라짐(디스크/HA에 영속화되지 않음). 매우 긴 지연이나 재부팅을 넘나드는 예약이 필요하면 추후 HA 자체 타이머 헬퍼나 스크립트 연동으로 개선 필요

## 1.1.0-beta.93

### 추가 (Feature) — 한 문장에 여러 기기를 동시에 지시하는 복합 명령 지원
- **재현**: "안방 등 켜고 커튼 닫아" 같은 명령은 조명만 켜지고 커튼은 무시되거나 그 반대였음 -- `execute_device_control_intent()`가 문장 전체에 대해 켜짐/꺼짐/열림/닫힘 동사를 딱 한 번만 판정하고, 커튼/조명/선풍기 등 7개 분기 중 처음 매치되는 것 하나만 실행하고 즉시 반환했기 때문
- **수정**: 신설 `_split_compound_clauses()`가 "켜고/끄고/닫고/열고/틀고/올리고/내리고/시작하고/..." 같은 "-고" 연결어 기준으로 문장을 절 단위로 분리("안방 등 켜고 커튼 닫아" -> ["안방 등 켜고", "커튼 닫아"]), 기존 단일 명령 처리 함수(`_execute_single_control_clause`로 이름 변경)를 절마다 각각 실행 후 결과를 합쳐서 응답. 나중 절이 방을 따로 말하지 않으면 그 앞에서 마지막으로 언급된 방을 그대로 이어받음("안방 등 모두 끄고 선풍기 끄고 커튼 닫아" 전부 안방 기준으로 처리)
- **부가 발견 및 수정**: 켜다/틀다/올리다/내리다/닫다/열다 같은 으-불규칙 동사는 "-고"형 어간(끄고/틀고/올리고/내리고/닫고/열고)이 기존 켜짐/꺼짐 판정 키워드(꺼/틀어/올려/내려/닫아/열어)의 부분 문자열이 아니어서, 위 분리 기능이 없어도 "끄고"처럼 절 끝에 "-고"가 오는 문장은 원래도 인식되지 않는 잠재 버그였음 -- 네 가지 판정 키워드 목록 모두에 "-고"형을 추가
- 서로 다른 기기가 같은 동사 어휘(켜다/끄다)를 반대 의미로 쓰는 문장("선풍기 켜고 에어컨 꺼")도 절 단위로 분리해서 판정하므로 올바르게 구분됨(문장 전체에 대해 켜짐/꺼짐을 한 번만 판정했다면 두 기기 다 켜졌을 상황)

## 1.1.0-beta.92

### 수정 (Fix) — 환경 분석 리포트가 상태표시 LED를 "켜진 조명"으로 집계하던 문제
- beta.91에서 날씨 질문의 기본 응답이 된 `get_ai_deep_environment_analysis()`와 기존 `get_comprehensive_home_summary()`의 점등 조명 집계가 "작은방 선풍기 Indicator Light" 같은 light 도메인 상태표시 LED까지 세고 있었음 (beta.86에서 기기 제어 쪽에만 적용했던 `DIAGNOSTIC_EXCLUDE_KEYWORDS` 필터가 리포트 집계 쪽엔 빠져 있었음)
- 두 함수의 점등 조명 목록에도 동일한 `DIAGNOSTIC_EXCLUDE_KEYWORDS` 필터 적용. 라이브 확인: "오늘 날씨 어때" 응답에서 "점등 조명: 총 2개 (gateway ligh, 작은방 선풍기 Indicator Light)"로 표시되던 것을 재현 후 수정

## 1.1.0-beta.91

### 변경 (Change) — "AI 딥 브레인" 모드 제거, 고속 제어 모드로 통합
- **배경**: 실행 모드 선택기에 있던 두 모드("고속 제어 모드"와 "고속 제어 & 스마트 모드")는 실제로는 기기 제어·조회 등 대부분의 요청에서 완전히 동일한 `handle_agent_chat()` 로직을 그대로 호출했고(`core/streamer.py`), 유일한 차이는 날씨/환경/상태 계열 질문에서 후자만 `get_ai_deep_environment_analysis()`(실외 대비 실내 매트릭스 + AI 추천)를 썼다는 점뿐이었음. 둘 다 실제 LLM 호출이 없는 로컬 휴리스틱이라 속도·토큰 비용 차이도 없어서, 별도 모드로 나눠 사용자가 매번 골라야 할 이유가 없었음
- **변경**: 날씨 관련 질문(branch 6: 날씨/기상/일기예보/온습도/대시보드)에서 `get_ai_deep_environment_analysis()`를 기본으로 쓰도록 `core/ha_engine.py`에 흡수 -- 비용 없이 항상 더 풍부한 답을 줌. "우리집 상황 어때?" 같은 폭넓은 상태 질문(branch 17 + Fallback)은 재실/커튼/팬/시스템 리소스까지 포괄하는 기존 `get_comprehensive_home_summary()`를 그대로 유지(환경 분석 전용 함수로 바꾸면 오히려 커버리지가 줄어들어서 유지)
- 실행 모드 선택기에서 "고속 제어 & 스마트 모드"(구 AI 딥 브레인) 항목 제거, 남은 항목의 짧은 이름을 "고속"→"제어"로 변경 (긴 이름 "고속 제어 모드"는 그대로) -- `core/ui/scripts.py`의 `STREAM_MODES`
- `stream_ai_deep_brain()`(`core/streamer.py`)과 그 전용 기록 함수 `record_mode1_interaction()`(`core/session_manager.py`), 더 이상 쓰이지 않게 된 `get_weather_env_summary()`(`core/renderers.py`) 완전히 삭제. CLI(Mode 3) 하드웨어 미지원/기동 실패 시 자동 전환 폴백 3곳(전부 `stream_ai_deep_brain` 호출)은 `stream_fast_dashboard`로 교체
- 도움말 패널의 실행 모드 설명, 기능 소개 응답("N대 스트리밍 모드") 등 관련 UI 문구 정리

## 1.1.0-beta.90

### 추가 (Feature) — 방/기기 이름 없는 바로 다음 턴 명령("닫아")이 직전 대상을 이어받도록 개선
- **재현**: "안방커튼 열어" 다음 턴에 기기명 없이 그냥 "닫아"만 말하면 못 알아들음 -- `execute_device_control_intent()`의 7개 분기 모두 커튼/조명/선풍기 같은 기기 키워드가 있어야 진입하는데, "닫아"만 있는 문장엔 그런 키워드가 전혀 없어 아무 분기도 안 걸리고 beta.86의 "확인하지 못했습니다" 재질문으로 빠졌음
- **수정**: `resolve_control_scope()`가 성공적으로 대상을 확정할 때마다(`core/session_manager.py`의 `set_last_control_targets`) 그 엔티티 id들을 대화별로 기억해두고, 이후 턴에서 기기 키워드가 전혀 매치되지 않았는데 켜/꺼/열어/닫아 같은 동작 동사만 있으면(`execute_device_control_intent()` 끝의 새 폴백) 직전에 기억해둔 엔티티에 이번 동사에 맞는 서비스(`open_cover`/`close_cover`는 cover 도메인, 나머지는 `turn_on`/`turn_off`)를 그대로 적용
- 커튼→커튼, 선풍기 풍량 설정→끄기, 조명 켜기→끄기 등 도메인이 섞이지 않는 한 모든 기기 종류에 공통 적용됨 (단위 테스트로 확인). 대화 맥락이 없거나(첫 턴) 기기 키워드가 있는 명시적 명령은 기존 동작 그대로 유지

### 추가 (Feature) — 조명 밝기 퍼센트(%) 직접 지정
- "안방 책상 등 50%"도 beta.87의 선풍기 풍량(%)과 동일한 구조의 문제였음 -- 켜/꺼 동사가 없어 제어 명령으로 인식되지 않고 무관한 종합 브리핑으로 빠졌음
- `execute_device_control_intent()`의 조명 분기에도 동일한 퍼센트 감지(`_PERCENT_RE`)를 추가 -- 매치되면 on/off 토글 대신 `light.turn_on`을 `brightness_pct`와 함께 호출해 밝기를 정확히 설정(끄고 켜는 토글과 달리 밝기 지정과 동시에 켜짐). 0%는 "켜짐" 상태로서 의미가 없어 1~100으로 clamp
- 트리거 자체(숫자+%/퍼센트/프로 패턴)는 beta.87에서 이미 `core/ha_engine.py`에 추가돼 있어 별도 수정 불필요 -- 조명 분기만 추가
- 실제 기기(`light.mi_stand_light`, "안방 책상 등")로 라이브 검증 완료

## 1.1.0-beta.88

### 수정 (Fix) — CLI 모드(3모드)에서 중지 버튼을 눌러도 뒤늦게 기기 제어가 실행되던 문제
- **재현**: "안방 다운라이트 켜"를 요청한 뒤 응답이 끝나기 전에 중지 버튼을 누르면 화면엔 즉시 "중지되었습니다"가 표시되지만, 몇 초 후 다운라이트가 꺼지는 등 중지 이후에도 HA 제어가 실제로 실행됨
- **원인**: `stop_stream()`이 `script`/`agy` 프로세스 그룹에 SIGTERM만 보내고 끝났음(`core/streamer.py`) -- `agy`는 Go 바이너리라 SIGTERM을 받아도 즉시 죽지 않고 이미 시작한 정리 절차(그 시점에 진행 중이던 ha-mcp 도구 호출 등)를 마저 끝내는 "graceful shutdown"을 수행할 수 있어, 중지 직후 실행 중이던 조명 제어 호출이 그대로 완료될 수 있었음. 게다가 `os.killpg` 자체가 실패했을 때의 예외 폴백은 `proc.kill()`로 `script` 래퍼 프로세스만 죽이고 `agy`/그 하위 ha-mcp 프로세스는 그대로 살려두는 허점도 있었음
- **수정**: SIGTERM 전송은 유지하되, 1.5초 유예 후에도 프로세스 그룹이 살아 있으면 그룹 전체(`os.killpg(pgid, SIGKILL)`)를 강제 종료하도록 에스컬레이션 추가 -- 정상 종료 시엔 기존과 동일하게 조용히 끝나고, graceful shutdown이 길어져 도구 호출을 이어가려는 경우에만 하드킬이 개입해 중지 이후 추가 제어가 실행되는 것을 차단

## 1.1.0-beta.87

### 추가 (Feature) — 선풍기/환풍기 풍량 퍼센트(%) 직접 지정
- "안방 선풍기 50%로 해줘" 같은 명령은 켜/꺼 등 기존 제어 동사를 전혀 포함하지 않아 제어 명령으로 인식조차 안 됐고(beta.86의 "다운라이트" 사례와 동일한 구조의 문제), 결국 무관한 종합 상황 브리핑 Fallback으로 빠졌음
- `core/ha_engine.py`에 숫자+`%`/`퍼센트`/`프로` 패턴(`_PERCENT_PATTERN`)을 제어 명령 트리거에 추가해 이런 문장도 기기 제어 분기로 들어오도록 수정
- `execute_device_control_intent()`의 선풍기/환풍기 분기에 퍼센트 감지(`_PERCENT_RE`)를 추가 -- 매치되면 on/off 대신 `fan.set_percentage` 서비스를 호출해 정확히 요청한 값(0~100 범위로 clamp)으로 설정. 실제 기기(`fan.dmaker_p220_72de_fan`, 표준 `percentage` 속성 지원)로 라이브 검증 완료

## 1.1.0-beta.86

### 수정 (Fix) — "안방 다운라이트 꺼"가 조명 대신 집 전체 상황 브리핑을 응답하던 문제
- **재현**: 실제 등록된 조명 중 "안방 다운라이트"(`light.wiz_rgbw_tunable_ab09ac`), "안방 스텐드 램프"(`light.bed_stand_lamp`), "작은방 스트립라이트"처럼 이름에 "등"/"조명"/"전등"이 전혀 없는 기기들이 있음 -- `execute_device_control_intent()`의 조명 트리거 키워드가 `["불","조명","전등","등"]`뿐이라 이 기기들에 대한 명령은 조명 제어 분기 자체에 진입하지 못하고 빈 문자열을 반환했고, `core/ha_engine.py`의 제어 시도(자동화/스크립트/기기 제어)가 전부 실패하면서 코드가 그대로 통과해 맨 아래 "Fallback" 분기(`get_comprehensive_home_summary`, 🏠 우리집 스마트홈 종합 상황 브리핑)까지 흘러가 명령과 무관한 응답을 냈음
- **수정 1**: 조명 트리거 키워드에 `"라이트"`, `"램프"` 추가 (`execute_device_control_intent`, `get_device_status_answer`의 `_DOMAIN_STATUS_TRIGGERS`, 방별 조명 요약 분기 3곳 모두 동일 적용) — "다운라이트"/"스트립라이트"/"램프" 계열 실제 기기명을 인식
- **수정 2 (근본 원인 차단)**: `켜/꺼/...` 같은 제어 동사가 있었는데도 자동화/스크립트/기기 제어 세 가지 시도가 전부 실패하면, 이후 방 질문/날씨/상태 등 무관한 분기로 계속 흘러가다 맨 아래 종합 브리핑 Fallback에 도달하던 구조 자체를 차단 -- 이제 "무엇을 켜거나 끄시려는 건지 확인하지 못했습니다..." 재질문으로 즉시 응답. 앞으로 키워드 목록에 없는 새 기기명이 나와도 엉뚱한 응답 대신 재질문으로 안전하게 처리됨
- **부가 발견 및 수정**: 실제 기기 중 "안방 전기스토브1S Indicator Light", "작은방 선풍기 Indicator Light"처럼 조명(light.) 도메인으로 노출된 상태표시 LED가 있어 방 이름이 이름에 포함됨 -- "안방 불 다 꺼" 같은 방 전체 조명 명령에 휩쓸려 같이 꺼질 수 있었던 문제 발견. 기존에 switch 도메인에만 있던 `SWITCH_EXCLUDE_KEYWORDS`(Indicator/LED/표시등 등)를 `DIAGNOSTIC_EXCLUDE_KEYWORDS`로 이름을 바꾸고 light 도메인 후보 필터링(제어 + 상태 질문 둘 다)에도 동일하게 적용

## 1.1.0-beta.85

### 수정 (Fix) — 채팅 입력창 배경 텍스트를 한글로 변경
- "Ask anything, @ to mention, / for actions" -> "무엇이든 물어보세요"

## 1.1.0-beta.84

### 수정 (Fix) — 모바일에서 채팅 입력창 하단 모드/모델/에이전트 선택 버튼이 동작하지 않던 문제
- **원인**: beta.6x~77 구간(`feature/reasoning-timeline-mobile-ui` 브랜치)에서 모바일 화면(≤768px)에서 모드/모델/에이전트 버튼 3개가 화면 밖으로 밀려나던 문제를 고치기 위해 `.composer-toolbar-left`에 `overflow-x: auto`를 추가했는데, CSS 스펙상 `overflow-x`/`overflow-y` 중 하나만 `visible`이 아닌 값으로 지정하면 나머지 축도 자동으로 `auto`로 계산됨(`.model-dropdown-list` 주석에 이미 기록된 동일한 함정) — 이 때문에 위쪽으로 펼쳐지는(`position:absolute; bottom:calc(100% + 8px)`) 각 버튼의 드롭다운(`.model-dropdown`)이 부모 컨테이너에 의해 잘려서 안 보이게 됨. 버튼 자체는 클릭이 되지만 메뉴가 시각적으로 나타나지 않아 "선택이 안 되는" 것처럼 보였음
- **수정**: `core/ui/scripts.py`에 신설한 `positionDropdownAboveButton()`이 드롭다운을 열 때 버튼의 `getBoundingClientRect()` 기준으로 `position: fixed` + 뷰포트 좌표를 인라인으로 지정하도록 변경 — 스크롤 가능한 부모의 클리핑 범위를 완전히 벗어나므로 모바일에서도 정상적으로 펼쳐짐. 화면 오른쪽 경계를 넘지 않도록 `left` 값도 clamp
- 실행 모드(`toggleStreamModePicker`)/모델(`toggleModelPicker`)/에이전트(`toggleAgentPicker`) 픽커 3개 모두 동일하게 적용

## 1.1.0-beta.83

### 수정 (Fix) — 방에 같은 종류 기기가 여러 개일 때 "안방 등 켜"가 전부 켜지던 문제
- **재현**: 안방에 "안방 등", "안방 스탠드 등", "안방 화장대 등", "안방 화장대 다운라이트" 4개의 조명 엔티티가 있는 상태에서 "안방 등 켜"라고 명령하면 4개 모두 켜짐 — 사용자는 이름이 정확히 "안방 등"인 조명 하나만 의도했음
- **원인**: `resolve_control_scope()`가 방이 인식되면(beta.? 도입, `matched_room`) 그 방 이름을 포함하는 같은 도메인의 엔티티를 전부 대상으로 삼았고, 방 안에서 어떤 특정 기기를 말하는지는 전혀 좁히지 않았음 — "여러 방 중 어디?" 모호성만 가드되어 있었고 "한 방 안에 같은 종류가 여러 개일 때 어떤 것?"은 가드가 없었음
- **수정**: 신설 `_narrow_multi_device_targets()` — 방 이름을 뺀 나머지 이름("스탠드등", "화장대등", "화장대다운라이트")이 명령에 명시적으로 포함되면 그 기기 하나로 좁히고, 아무 수식어도 없으면(단순 "등"/"불"/"조명"/"전등") 나머지 부분이 없는(=이름이 가장 짧은/가장 단순한, 예: "안방 등") 기기를 기본값으로 선택. 여러 개가 똑같이 모호하면(예: "등1"/"등2") 추측하지 않고 "어느 것을 말씀하시는 걸까요? (... 중 하나로, 또는 '전체'/'다'라고 말씀해 주세요.)"로 재질문
- "안방 불 다 꺼줘" 같은 명시적 전체 키워드(`_has_whole_house_keyword`)는 기존과 동일하게 방 안의 모든 기기를 대상으로 함 — 이번 수정은 그 키워드가 없을 때만 적용
- 조명뿐 아니라 `resolve_control_scope`를 공유하는 커튼/팬/가습기·제습기/보일러 등 스위치/에어컨/미디어 제어에도 동일하게 적용됨
