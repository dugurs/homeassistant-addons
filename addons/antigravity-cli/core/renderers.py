...
from core.ha_client import DIAGNOSTIC_EXCLUDE_KEYWORDS
...
def get_ai_deep_environment_analysis(states: list, prompt: str = "", is_mobile: bool = False) -> str:
    """Deep environmental analysis & dynamic contextual synthesis -- outdoor
    weather vs. per-room indoor comfort/air-quality matrix plus AI-style
    recommendations. Used for weather-dashboard queries (see handle_agent_chat
    branch 6 in core/ha_engine.py); this used to be exclusive to a separate
    standalone "AI Deep Brain" mode, folded into the single control mode
    once that mode was removed (no real cost difference between the two --
    neither ever called an LLM)."""
    outdoor_temp = 27.0
    outdoor_hum = 66
    outdoor_pm25 = None
    weather_cond = "cloudy"
    ...
    for s in states:
        ...
        if eid.startswith("light.") and s.get("state") == "on":
            if "all" in eid.lower() or "전체" in fn:
                continue
            if any(x in fn for x in DIAGNOSTIC_EXCLUDE_KEYWORDS):
                continue
            on_lights.append(fn)
    if on_lights:
